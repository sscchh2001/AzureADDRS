...
$global:enterprise= $global:Bypass.Contains("*.partner.microsoftonline.cn") -or $global:Bypass.Contains("enterpriseregistration.partner.microsoftonline.cn")
...
if ($ProxyServer -eq "NoProxy"){
    $PSScript = "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; (Invoke-WebRequest -uri 'https://login.partner.microsoftonline.cn' -UseBasicParsing).StatusCode"
    $TestResult = RunPScript -PSScript $PSScript
    if ($TestResult -eq 200){
        Write-Host "Connection to login.partner.microsoftonline.cn .............. Succeeded." -ForegroundColor Green 
    }else{
        $TestFailed=$true
        Write-Host "Connection to login.partner.microsoftonline.cn ................. failed." -ForegroundColor Red 
    }
    $PSScript = "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; (Invoke-WebRequest -uri 'https://device.login.partner.microsoftonline.cn' -UseBasicParsing).StatusCode"
    $TestResult = RunPScript -PSScript $PSScript
    if ($TestResult -eq 200){
        Write-Host "Connection to device.login.partner.microsoftonline.cn ......  Succeeded." -ForegroundColor Green 
    }else{
        $TestFailed=$true
        Write-Host "Connection to device.login.partner.microsoftonline.cn .......... failed." -ForegroundColor Red 
    }

    $PSScript = "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; (Invoke-WebRequest -uri 'https://enterpriseregistration.windows.net/microsoft.com/discover?api-version=1.7' -UseBasicParsing -Headers @{'Accept' = 'application/json'; 'ocp-adrs-client-name' = 'dsreg'; 'ocp-adrs-client-version' = '10'}).StatusCode"
    $TestResult = RunPScript -PSScript $PSScript
    if ($TestResult -eq 200){
    ...
    $PSScript = "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; (Invoke-WebRequest -uri 'https://enterpriseregistration.partner.microsoftonline.cn/mcpod.partner.onmschina.cn/discover?api-version=1.7' -UseBasicParsing -Headers @{'Accept' = 'application/json'; 'ocp-adrs-client-name' = 'dsreg'; 'ocp-adrs-client-version' = '10'}).StatusCode"
    $TestResult = RunPScript -PSScript $PSScript
    if ($TestResult -eq 200){
        Write-Host "Connection to enterpriseregistration.partner.microsoftonline.cn ..... Succeeded." -ForegroundColor Green 
    }else{
        $TestFailed=$true
        Write-Host "Connection to enterpriseregistration.partner.microsoftonline.cn ........ failed." -ForegroundColor Red 
    }
}else{
    if ($global:login){
        $PSScript = "(Invoke-WebRequest -uri 'https://login.partner.microsoftonline.cn' -UseBasicParsing).StatusCode"
        $TestResult = RunPScript -PSScript $PSScript
    }else{
        $PSScript = "(Invoke-WebRequest -uri 'https://login.partner.microsoftonline.cn' -UseBasicParsing -Proxy $ProxyServer).StatusCode"
        $TestResult = RunPScript -PSScript $PSScript
    }
    if ($TestResult -eq 200){
        Write-Host "Connection to login.partner.microsoftonline.cn .............. Succeeded." -ForegroundColor Green 
    ...
    if ($global:device){
        $PSScript = "(Invoke-WebRequest -uri 'https://device.login.partner.microsoftonline.cn' -UseBasicParsing).StatusCode"
        $TestResult = RunPScript -PSScript $PSScript
    }else{
        $PSScript = "(Invoke-WebRequest -uri 'https://device.login.partner.microsoftonline.cn' -UseBasicParsing -Proxy $ProxyServer).StatusCode"
        $TestResult = RunPScript -PSScript $PSScript
    }
    if ($TestResult -eq 200){
        Write-Host "Connection to device.login.partner.microsoftonline.cn ......  Succeeded." -ForegroundColor Green 
    ...
    if ($global:enterprise){
        $PSScript = "(Invoke-WebRequest -uri 'https://enterpriseregistration.partner.microsoftonline.cn/mcpod.partner.onmschina.cn/discover?api-version=1.7' -UseBasicParsing -Headers @{'Accept' = 'application/json'; 'ocp-adrs-client-name' = 'dsreg'; 'ocp-adrs-client-version' = '10'}).StatusCode"
        $TestResult = RunPScript -PSScript $PSScript
    }else{
        $PSScript = "(Invoke-WebRequest -uri 'https://enterpriseregistration.partner.microsoftonline.cn/mcpod.partner.onmschina.cn/discover?api-version=1.7' -UseBasicParsing -Proxy $ProxyServer -Headers @{'Accept' = 'application/json'; 'ocp-adrs-client-name' = 'dsreg'; 'ocp-adrs-client-version' = '10'}).StatusCode"
        $TestResult = RunPScript -PSScript $PSScript
    }
    if ($TestResult -eq 200){
        Write-Host "Connection to enterpriseregistration.partner.microsoftonline.cn ..... Succeeded." -ForegroundColor Green 
